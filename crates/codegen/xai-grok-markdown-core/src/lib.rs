//! Headless markdown analysis sharing Olympus Build's exact `pulldown-cmark` config.
//!
...
/// The exact `pulldown-cmark` option set Olympus Build uses to render markdown.
///
...
/// Parse `text` with Olympus Build's options; count elements and flag structural issues.
