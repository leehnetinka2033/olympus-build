...
pub fn print_update_status(status: &UpdateStatus, json: bool) -> anyhow::Result<()> {
    ...
    if status.update_available {
        if let Some(latest_version) = status.latest_version.as_deref() {
            println!(
                "A new version of Olympus Build is available: {} -> {}{}",
                status.current_version, latest_version, channel_label
            );
        } else {
            println!("A new version of Olympus Build is available.");
        }
        return Ok(());
    }
    ...
    if let Some(latest_version) = status.latest_version.as_deref() {
        println!(
            "Olympus Build - v{} (latest: {}){}",
            status.current_version, latest_version, channel_label
        );
        return Ok(());
    }

    println!("Olympus Build - v{}{}", status.current_version, channel_label);
    Ok(())
}
