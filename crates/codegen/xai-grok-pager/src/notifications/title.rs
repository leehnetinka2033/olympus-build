...
impl TitleManager {
    ...
    pub fn update(&mut self, state: &TitleState<'_>) -> Option<String> {
        ...
        if !has_parts {
            self.composed.clear();
            self.composed.push_str("olympus");
        }
    ...
    pub fn reset(&mut self) -> String {
        let esc = build_title_escape("olympus");
        self.last_title.clear();
        self.last_title.push_str("olympus");
...
mod tests {
    ...
    fn grok_only_produces_just_grok() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "olympus");
    }
    ...
    fn reset_clears_state_and_emits_grok() {
        ...
        mgr.update(&state);
        assert_ne!(mgr.last_title, "olympus");
    ...
    fn default_config_active_turn_with_permissions() {
        ...
        for t in [&t1, &t2] {
            assert!(t.contains("olympus"), "title missing 'grok': {t}");
